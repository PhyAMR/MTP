...
Notice that in these examples we have not defined a prob. measure, which is not
...
\begin{enumerate}
  ...
  \item Connection with the diffusion equation
    ...
    \begin{DispWithArrows}[displaystyle, format=c]
      w\left(x, t \mid x_{0}, t_{0}\right)=\frac{1}{\sqrt{2 \pi\left(t-t_{0}\right)}} e^{-\frac{\left(x-x_{0}\right)^{2}}{2\left(t-t_{0}\right)}}
    \end{DispWithArrows}
...
We can add a drift $\\mu$ and change the diffusion coefficient of a Brownian
...
\begin{DispWithArrows}[displaystyle, format=ll]
  \begin{aligned}
    x\left(t_{i}\right) & =x\left(t_{i-1}\right)+\mu \Delta t+\sqrt{2D \Delta t} \varepsilon_{i} \\
    x(0) & =x_{0}
...
\begin{DispWithArrows}[displaystyle, format=c]
  ...
  \mid x_{0}, t_{0}\right)=\frac{1}{\sqrt{2 \pi D\left(t_{1}-t_{0}\right)}} e^{-\frac{\left(x_{1}-x_{0}\right)^{2}}{2D\left(t_{1}-t_{0}\right)}}
\end{DispWithArrows}
...
\begin{enumerate}
  \item Show that the Poisson process defined by
    \begin{DispWithArrows}[displaystyle, format=c]
      P_{1|1}
      \left(x_{2} t_{2}
      \mid x_{1} t_{1}\right)=\frac{\left(\lambda\left(t_{2}-t_{1}\right)\right)^{x_{2}-x_{1}}}{\left(x_{2}-x_{1}\right)\!} e^{-\lambda\left(t_{2}-t_{1}\right)} \quad t_{2}>t_{1}
    \end{DispWithArrows}
    and
