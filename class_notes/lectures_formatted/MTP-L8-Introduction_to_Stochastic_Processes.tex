...
\begin{enumerate}
  ...
  \item Connection with the diffusion equation
    ...
    \begin{DispWithArrows}[displaystyle, format=c]
      w\left(x, t \mid x_{0}, t_{0}ight)=\frac{1}{\sqrt{2 \pi\left(t-t_{0}ight)}} e^{-\frac{\left(x-x_{0}ight)^{2}}{2\left(t-t_{0}ight)}}
    \end{DispWithArrows}
  ...
  \item Correlation function
    ...
    $\\mathbb{E}[B(s)]=0, \\mathbb{E}\\left[B(s)^{2}ight]=s$. Then
    \begin{DispWithArrows}[displaystyle, format=ll]
      \begin{aligned}
    ...
    \end{DispWithArrows}
    Exercises:
    i) (Ornstein-Uhlenbeck process). Define $V(t)=e^{-t} B\left(e^{2 t}ight)$
    ...
    iii) Show that
    $\\mathbb{E}\\left[e^{i \lambda B(t)}ight]=e^{-\frac{\lambda^{2} t}{2}}$ for
...
\begin{DispWithArrows}[displaystyle, format=ll]
  \begin{aligned}
    x\left(t_{i}\right) & =x\left(t_{i-1}ight)+\\mu \\Delta t+\sqrt{2D \\Delta t} \varepsilon_{i} \\
    x(0) & =x_{0}
